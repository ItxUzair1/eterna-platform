...
function priceDataForPlan(plan) {
  switch (plan) {
    ...
    case 'enterprise_seats':
      return { unit_amount: 3000, product_data: { name: 'Enterprise (per seat)' }, recurring: { interval: 'month' }, currency: 'usd' };
...
async function startCheckout({ tenantId, plan, seats = 1, addons = {} }) {
  ...
  const lineItems = [
    {
      price_data: priceDataForPlan(plan),
      quantity: plan === 'enterprise_seats' ? Math.max(1, Number(seats || 1)) : 1,
    },
  ];
  if (addons?.storageGB) {
    lineItems.push({
      price_data: { unit_amount: 100, product_data: { name: 'Storage Addon (per GB)' }, recurring: { interval: 'month' }, currency: 'usd' },
      quantity: Math.max(1, Number(addons.storageGB)),
    });
  }
...
async function upsertSubscriptionFromStripe(stripeData) {
  ...
  const plan = (() => {
    ...
    if (primary === process.env.PRICE_ENTERPRISE_SEAT) return 'enterprise_seats';
  ...
  const data = {
    ...
    seatsEntitled: plan === 'enterprise_seats' ? Number(seatsQuantity) : 1,
    storageEntitledGB: Number(storageAddonQty || 0) + 5,
  };

  const existing = await prisma.subscription.findFirst({ where: { tenantId, stripeSubId: stripeData.id } });
  if (existing) {
